...
static NTSTATUS WS2_async_recv( void* user, IO_STATUS_BLOCK* iosb, NTSTATUS status, ULONG_PTR *total )
{
    ws2_async* wsa = user;
    int result = 0, fd;
	NTSTATUS Status = iosb->async_status;
    sigset_t block_alarm;

	LOG(LOG_FILE, 0, 0, "Status %x\n", Status);

    sigemptyset(&block_alarm);
    sigaddset(&block_alarm, 60);
	...
	SERVER_START_REQ(async_set_result)
	{
		...
		req->event  = wsa->user_overlapped ? wsa->user_overlapped->hEvent : NULL;
		req->cvalue = (wsa->user_overlapped && ((unsigned long)wsa->user_overlapped->hEvent & 1) == 0) ?
			(unsigned long)(wsa->user_overlapped) : 0;
		req->status = Status;
		req->total  = result;
		req->callback = ws2_async_apc;
		req->arg1   = wsa;
		req->arg2   = iosb;
		wine_server_call(req);
	}
 ...
 */
static NTSTATUS WINAPI WS2_async_accept_recv( void *arg, IO_STATUS_BLOCK *iosb, NTSTATUS status, ULONG_PTR *total )
{
    struct ws2_accept_async *wsa = arg;
	NTSTATUS Status = iosb->async_status;
    sigset_t block_alarm;

	LOG(LOG_FILE, 0, 0, "wsa->read %p, hEvent %p\n", wsa->read, wsa->user_overlapped->hEvent);
    ...
    if (!wsa->read)
    {
		...
		wsa->read->user_overlapped = wsa->user_overlapped;
		wsa->read->completion_func = NULL;
    }

	iosb->async_status = Status;
    Status = WS2_async_recv( wsa->read, iosb, Status, total );
    if (Status == STATUS_PENDING)
        return Status;
...
finish:
	...
	/* We have free wsa->read in ws2_async_apc */
...
static NTSTATUS WINAPI WS2_async_accept( void *arg, IO_STATUS_BLOCK *iosb, NTSTATUS status )
{
	...
	LOG(LOG_FILE, 0, 0, "Status %x, hEvent %p, data_len %d\n", Status, wsa->user_overlapped->hEvent, wsa->data_len);
    ...
    *(int *)addr = len;
    ...
    SERVER_START_REQ( register_async )
    {
        ...
        req->handle   = ( SOCKET2HANDLE(wsa->s_accept) );
        req->async.callback = ( WS2_async_accept_recv );
        req->async.iosb     = ( iosb );
        req->async.arg      = ( wsa );
    ...
    if (Status != STATUS_PENDING)
    {
		...
		LOG(LOG_FILE, 0, 0, "Could not register async read, %x\n", Status);
        goto error;
    }
...
finish:
	...
	LOG(LOG_FILE, 0, Status, "return %x\n", Status);
    return Status;
}
...
extern void NtCatchApc(int param);
/***********************************************************************
 *		accept		(WS2_32.1)
 */
                      ...
                      DWORD rem_addr_len, LPDWORD received, LPOVERLAPPED overlapped )
{
	...
	LOG(LOG_FILE, 0, 0, "listerner %p, acceptor %p, dest_len %d, local_addr_len %d, rem_addr_len %d\n", 
			listener, acceptor, dest_len, local_addr_len, rem_addr_len);
	signal(SIGUSR2, NtCatchApc);
    ...
    {
        req->data.handle   = ( SOCKET2HANDLE(wsa->s_listen) );
        req->ahandle       = ( SOCKET2HANDLE(wsa->s_accept) );
        req->data.callback = ( WS2_async_accept );
        req->data.iosb     = ( wsa->user_overlapped );
        req->data.arg      = ( wsa );
        req->data.cvalue   = 0;
        status = wine_server_call( req );
    }
                                ...
                                LPDWORD lenstr )
{
    ...
    case WS_AF_INET6:
    {
        ...
        if ((sockaddr6->sin6_scope_id))
            sprintf(buffer+strlen(buffer), "%%%u", (unsigned int)sockaddr6->sin6_scope_id);
        if ((sockaddr6->sin6_port))
