...
static void delete_view( struct file_view *view ) /* [in] View */
{
    if (!(view->flags & VFLAG_SYSTEM)) {
        ...
        NtFreeVirtualMemory( NtCurrentProcess(), (PVOID *)(&view->base), (SIZE_T *)(&view->size), MEM_SYSTEM);
    }
                          ...
                          int top_down, BYTE vprot )
{
    ...
    status = NtAllocateVirtualMemory( NtCurrentProcess(), &ptr, 0, (SIZE_T *)(&size),
...
{
    NTSTATUS status;
	LOG(LOG_FILE, 0, 0, "process %p, ret %p, size %lx\n", process, *ret, *size_ptr);
	...
	LOG(LOG_FILE, 0, status, "return %x, ret %p\n", status, *ret);
    return status;
}
...
{
    NTSTATUS status;
	LOG(LOG_FILE, 0, 0, "process %p, addr %p\n", process, *addr_ptr);
...
{
    NTSTATUS ret;
	LOG(LOG_FILE, 0, 0, "handle %p, process %p, addr %p\n", handle, process, *addr_ptr);
    __asm__ __volatile__ (
            "movl $0x52,%%eax\n\t"
            "lea 8(%%ebp),%%edx\n\t"
            "int $0x2E\n\t"
