...
{
    NTSTATUS ret;
    LOG(LOG_FILE, 0, 0, "attr %p, name %s\n", attr, attr ? debugstr_w(attr->ObjectName->Buffer) : NULL);
    ...
    SERVER_END_REQ;
    TRACE("<- %p\n", *retkey);
    LOG(LOG_FILE, 0, ret, "return %x, retkey %p\n", ret, retkey ? *retkey : NULL);
    return ret;
}
...
NTSTATUS WINAPI NtDeleteValueKey( HANDLE hkey, const UNICODE_STRING *name )
{
    ...
    LOG(LOG_FILE, 0, 0, "hkey %p, name %s\n", hkey, debugstr_us(name));
    TRACE( "(%p,%s)\n", hkey, debugstr_us(name) );
                                 ...
                                 void *info, DWORD length, DWORD *result_len )
{
	...
	LOG(LOG_FILE, 0, 0, "handle %p, name %s\n", handle, debugstr_us(name));
	...
	OUT PHANDLE KeyHandle)	      /* [out] handle of HKEY_CURRENT_USER */
{
    ...
    LOG(LOG_FILE, 0, 0, "done handle %p\n", KeyHandle ? *KeyHandle : NULL);
	return ret;
}
