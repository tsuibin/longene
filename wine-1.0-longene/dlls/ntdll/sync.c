	...
	IN BOOLEAN InitialState)
{
    ...
    LOG(LOG_FILE, 0, 0, "%s, Reset %d, State %d\n",
		attr ? attr->ObjectName ? debugstr_w(attr->ObjectName->Buffer) : NULL : NULL, ManualReset, InitialState );
    __asm__ __volatile__ (
            "movl $0x17,%%eax\n\t"
            "lea 8(%%ebp),%%edx\n\t"
            "int $0x2E\n\t"
	...
	IN const OBJECT_ATTRIBUTES *attr )
{
    ...
    LOG(LOG_FILE, 0, 0, "%s\n",
		attr ? attr->ObjectName ? debugstr_w(attr->ObjectName->Buffer) : NULL : NULL);
    __asm__ __volatile__ (
            "movl $0x56,%%eax\n\t"
            "lea 8(%%ebp),%%edx\n\t"
            "int $0x2E\n\t"
                               ...
                               IN BOOLEAN InitialOwner)
{
    ...
    LOG(LOG_FILE, 0, status, "return %x, handle %p\n", status, MutantHandle ? *MutantHandle : NULL);
    return status;
}
...
#define ObjectHandleInformation ObjectDataInformation

void obj_type(DWORD count, const HANDLE *handles)
{
		...
		for (i=0; i<count; i++)
		{
			NtQueryObject(handles[i], ObjectBasicInformation, &ObjectInfo, sizeof(ObjectInfo), NULL);
			type = ObjectInfo.Reserved[0];
			s = buf + strlen(buf);
			sprintf(s, " %p=%d", handles[i], type);
			if (type==115) 
			{
				s = buf + strlen(buf);
				sprintf(s, "(MSG_QUEUE)");	
                                          ...
                                          const LARGE_INTEGER *timeout )
{
    ...
    if ((!count && handles) || count > MAXIMUM_WAIT_OBJECTS)
	...
	if (count) {
		LOG(LOG_FILE, 0, 0, "count %d, handle %p, timeout %p, %llx\n", count, *handles, timeout, timeout ? timeout->QuadPart : 0);
		obj_type(count, handles);  /* for debug */
	} else
		LOG(LOG_FILE, 0, 0, "count %d, timeout %p, %llx\n", count, timeout, timeout ? timeout->QuadPart : 0);
    do{
        __asm__ __volatile__ (
                    "movl $0xDE,%%eax\n\t"
                    "lea 8(%%ebp),%%edx\n\t"
    ...
    } while (-EINTR == ret || STATUS_USER_APC == ret);
    
	if (ret == STATUS_TIMEOUT)
	{
		LOG(LOG_FILE, 0, ret, "count=%d, TIMEOUT\n", count);
	}
	else
		LOG(LOG_FILE, 0, ret, "count=%d, return %x\n", count, ret);
                                                ...
                                                BOOLEAN alertable, const LARGE_INTEGER *timeout )
{
    ...
    LOG(LOG_FILE, 0, 0, "\n");
    obj_type(1, &hSignalObject); 	/* for debug */
    obj_type(1, &hWaitObject);
...
NTSTATUS WINAPI NtDelayExecution( BOOLEAN alertable, const LARGE_INTEGER *timeout )
{
    ...
    if (alertable)
		return NtWaitForMultipleObjects(0, NULL, TRUE, alertable, timeout);
