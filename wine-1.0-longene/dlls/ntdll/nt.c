	...
	HANDLE *TokenHandle)
{
    ...
    SERVER_END_REQ;
    LOG(LOG_FILE, 0, ret, "return %x, token %p\n", ret, TokenHandle ? *TokenHandle : NULL);
	...
	PULONG retlen )
{
    ...
    LOG(LOG_FILE, 0, 0, "token %p, class %d, info %p, length %d, ret %p)\n",
	...
	OUT PULONG ResultLength)
{
    ...
    LOG(LOG_FILE, 0, 0, "(0x%08x,%p,0x%08x,%p)\n",
