    ...
    while (import_list->u1.Ordinal)
    {
        ...
        else  /* import by name */
        {
            ...
            thunk_list->u1.Function = (ULONG_PTR)find_named_export( imp_mod, exports, exp_size,
                   (const char*)pe_name->Name, pe_name->Hint, load_path );
            if (!thunk_list->u1.Function)
            {
                thunk_list->u1.Function = allocate_stub( name, (const char*)pe_name->Name );
...
void *find_builtin_symbol(const char *dll_name, const char *symbol)
{
    ...
    dll_handle = get_dll_handle((char *)dll_name);
    ret =  wine_dlsym(dll_handle, symbol, error, err_size);
    return ret;
}
...
void WINAPI LdrInitializeThunk( ULONG unknown1, ULONG unknown2, ULONG unknown3, ULONG unknown4 )
{
    ...
    LOG(LOG_FILE, 0, 0, "LdrInitializeThunk()\n");
    ...
    LOG(LOG_FILE, 0, 0, "LdrInitializeThunk() done\n");
    return;
