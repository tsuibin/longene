                              ...
                              ULONG options, PVOID ea_buffer, ULONG ea_length )
{
    ...
    LOG(LOG_FILE, 0, 0, "handle=%p access=%08x name=%s objattr=%08x root=%p sec=%p io=%p alloc_size=%p\n"
	...
	LOG(LOG_FILE, 0, io->u.Status, "return %x, handle %p\n", io->u.Status, handle ? *handle : NULL);
    return io->u.Status;
}
