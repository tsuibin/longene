...
void security_set_thread_token(struct w32thread *thread, obj_handle_t handle)
{
	if (!handle) {
		if (thread->token)
		...
		struct token *token = (struct token *)get_handle_obj(handle, TOKEN_IMPERSONATE);
		if (token) {
			if (thread->token)
				release_object(thread->token);
			thread->token = token;
...
uid_t uk_getuid(void)
{
	return (uid_t)0;    /* should be sys_getuid()? */
}
...
static struct privilege *privilege_add(struct token *token, const LUID *luid, int enabled)
{
	...
	if (privilege) {
		INIT_LIST_HEAD(&privilege->entry);
		privilege->luid = *luid;
		privilege->def = privilege->enabled = (enabled != 0);
		list_add_before(&token->privileges, &privilege->entry);
	}
	return privilege;
}
...
{
	struct token *token = alloc_wine_object(&token_ops);
	if (token)
	{
		unsigned int i;

		INIT_DISP_HEADER(&token->obj.header, _TOKEN, 
				sizeof(struct token) / sizeof(ULONG), 0);
		allocate_luid(&token->token_id);
		if (modified_id)
			token->modified_id = *modified_id;
		else
			allocate_luid(&token->modified_id);
		INIT_LIST_HEAD(&token->privileges);
		INIT_LIST_HEAD(&token->groups);
		...
		for (i = 0; i < group_count; i++) {
			...
			memcpy(&group->sid, groups[i].Sid, FIELD_OFFSET(SID,
						SubAuthority[((const SID *)groups[i].Sid)->SubAuthorityCount]));
			group->enabled = TRUE;
			group->def = TRUE;
			...
			list_add_before(&token->groups, &group->entry);
				...
				int impersonation_level)
{
	...
	LIST_FOR_EACH_ENTRY(group, &src_token->groups, struct group, entry) {
		...
		list_add_before(&token->groups, &newgroup->entry);
	}
...
struct token *token_create_admin(void)
{
	...
	/* FIXME: should be randomly generated when tokens are inherited by new processes */
	static const unsigned int logon_subauth[] = {SECURITY_LOGON_IDS_RID, 0, 1};
	...
	default_dacl = create_default_dacl(&interactive_sid);
	...
	if (alias_admins_sid && alias_users_sid && logon_sid && default_dacl) {
		...
		token = create_token(TRUE, security_unix_uid_to_sid(uk_getuid()),
...
static int token_sid_present(struct token *token, const SID *sid, int deny)
{
	...
	if (security_equal_sid(token->user, sid))
		return TRUE;

	LIST_FOR_EACH_ENTRY(group, &token->groups, struct group, entry) {
		if (!group->enabled)
			continue;
		if (group->deny_only && !deny)
			continue;

		if (security_equal_sid(&group->sid, sid))
			return TRUE;
	}
		...
		switch (ace->AceType)
		{
			case ACCESS_DENIED_ACE_TYPE:
				...
				if (token_sid_present(token, sid, TRUE)) {
					...
					map_generic_mask(&access, (GENERIC_MAPPING *)mapping);
...
int check_object_access(struct object *obj, unsigned int *access)
{
	...
	ktrace("obj=%p ops=%p\n", obj, BODY_TO_HEADER(obj)->ops);
	mapping.GenericAll = BODY_TO_HEADER(obj)->ops->map_access(obj, GENERIC_ALL);
	...
	mapping.GenericRead  = BODY_TO_HEADER(obj)->ops->map_access((void*)obj, GENERIC_READ);
	mapping.GenericWrite = BODY_TO_HEADER(obj)->ops->map_access((void*)obj, GENERIC_WRITE);
	mapping.GenericExecute = BODY_TO_HEADER(obj)->ops->map_access((void*)obj, GENERIC_EXECUTE);
...
DECL_HANDLER(open_token)
{
	...
	if (req->flags & OPEN_TOKEN_THREAD) {
		struct w32thread *thread = get_thread_from_handle(req->handle, 0);
		if (thread) {
			if (thread->token) {
		...
		struct w32process *process = get_process_from_handle(req->handle, 0);
		if (process) {
			if (process->token)
...
DECL_HANDLER(adjust_token_privileges)
{
	...
	if ((token = (struct token *)get_handle_obj(req->handle, access))) {
...
DECL_HANDLER(get_token_privileges)
{
	...
	ktrace("\n");
	if ((token = (struct token *)get_handle_obj(req->handle, TOKEN_QUERY))) {
		int priv_count = 0;
		LUID_AND_ATTRIBUTES *privs;
		struct privilege *privilege;
...
DECL_HANDLER(duplicate_token)
{
	...
	ktrace("\n");
	if ((src_token = (struct token *)get_handle_obj(req->handle, TOKEN_DUPLICATE))) {
		...
		if (size_needed <= get_reply_max_size()) {
			...
			if (tg) {
				...
				LIST_FOR_EACH_ENTRY(group, &token->groups, const struct group, entry) {
					...
					*attr_ptr = 0;
					if (group->mandatory)
						*attr_ptr |= SE_GROUP_MANDATORY;
					if (group->def)
						*attr_ptr |= SE_GROUP_ENABLED_BY_DEFAULT;
					if (group->enabled)
						*attr_ptr |= SE_GROUP_ENABLED;
					if (group->owner)
						*attr_ptr |= SE_GROUP_OWNER;
					if (group->deny_only)
						*attr_ptr |= SE_GROUP_USE_FOR_DENY_ONLY;
					if (group->resource)
						*attr_ptr |= SE_GROUP_RESOURCE;
					if (group->logon)
						*attr_ptr |= SE_GROUP_LOGON_ID;
...
DECL_HANDLER(set_security_object)
{
	...
	if (!(obj = get_handle_obj(req->handle, access)))
		return;

	if (BODY_TO_HEADER(obj)->ops->set_sd)
		BODY_TO_HEADER(obj)->ops->set_sd(obj, sd, req->security_info);
	release_object(obj);
}
