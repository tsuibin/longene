...
struct console_input
{
	...
	struct kevent               *event;         /* event to wait on for input queue */
};
...
static int console_input_events_satisfied(struct object *obj, struct w32thread *thread);   
static void console_input_destroy(struct object *obj);
static int console_input_events_signaled(struct object *obj, struct w32thread *thread);
...
static const struct object_ops console_input_events_ops =
{
	...
	console_input_events_satisfied,              /* satisfied */
	no_signal,                 /* signal */
	default_get_sd,            /* get_sd */
	default_set_sd             /* set_sd */
};
...
static WCHAR console_input_name[] = {'C','o','n','s','o','l','e','_','I','n','p','u','t',0};
static WCHAR console_input_events_name[] =
    {'C','o','n','s','o','l','e','_','I','n','p','u','t','_','E','v','e','n','t','s',0};
static WCHAR screen_buffer_name[] = {'S','c','r','e','e','n','_','B','u','f','f','e','r',0};
...
POBJECT_TYPE console_input_object_type = NULL;
EXPORT_SYMBOL(console_input_object_type);

POBJECT_TYPE console_input_events_object_type = NULL;
EXPORT_SYMBOL(console_input_events_object_type);

POBJECT_TYPE screen_buffer_object_type = NULL;
EXPORT_SYMBOL(screen_buffer_object_type);

VOID
init_console_input_implement(VOID)
{
	OBJECT_TYPE_INITIALIZER ObjectTypeInitializer;
	UNICODE_STRING Name;

	memset(&ObjectTypeInitializer, 0, sizeof(ObjectTypeInitializer));
	init_unistr(&Name, console_input_name);
	ObjectTypeInitializer.Length = sizeof(ObjectTypeInitializer);
	ObjectTypeInitializer.DefaultNonPagedPoolCharge = sizeof(struct console_input);
	ObjectTypeInitializer.GenericMapping = mapping;
	ObjectTypeInitializer.PoolType = NonPagedPool;
	ObjectTypeInitializer.ValidAccessMask = EVENT_ALL_ACCESS;
	ObjectTypeInitializer.UseDefaultObject = TRUE;
	create_type_object(&ObjectTypeInitializer, &Name, &console_input_object_type);
}

VOID
init_console_input_events_implement(VOID)
{
	OBJECT_TYPE_INITIALIZER ObjectTypeInitializer;
	UNICODE_STRING Name;

	memset(&ObjectTypeInitializer, 0, sizeof(ObjectTypeInitializer));
	init_unistr(&Name, console_input_events_name);
	ObjectTypeInitializer.Length = sizeof(ObjectTypeInitializer);
	ObjectTypeInitializer.DefaultNonPagedPoolCharge = sizeof(struct console_input_events);
	ObjectTypeInitializer.GenericMapping = mapping;
	ObjectTypeInitializer.PoolType = NonPagedPool;
	ObjectTypeInitializer.ValidAccessMask = EVENT_ALL_ACCESS;
	ObjectTypeInitializer.UseDefaultObject = TRUE;
	create_type_object(&ObjectTypeInitializer, &Name, &console_input_events_object_type);
}

VOID
init_screen_buffer_implement(VOID)
{
	OBJECT_TYPE_INITIALIZER ObjectTypeInitializer;
	UNICODE_STRING Name;

	memset(&ObjectTypeInitializer, 0, sizeof(ObjectTypeInitializer));
	init_unistr(&Name, screen_buffer_name);
	ObjectTypeInitializer.Length = sizeof(ObjectTypeInitializer);
	ObjectTypeInitializer.DefaultNonPagedPoolCharge = sizeof(struct screen_buffer);
	ObjectTypeInitializer.GenericMapping = mapping;
	ObjectTypeInitializer.PoolType = NonPagedPool;
	ObjectTypeInitializer.ValidAccessMask = EVENT_ALL_ACCESS;
	ObjectTypeInitializer.UseDefaultObject = TRUE;
	create_type_object(&ObjectTypeInitializer, &Name, &screen_buffer_object_type);
}
...
static unsigned int console_map_access(struct object *obj, unsigned int access)
{
	if (access & GENERIC_READ)
		access |= SYNCHRONIZE | STANDARD_RIGHTS_READ | CONSOLE_READ;
	if (access & GENERIC_WRITE)
		access |= SYNCHRONIZE | STANDARD_RIGHTS_WRITE | CONSOLE_WRITE;
	if (access & GENERIC_EXECUTE)
		access |= SYNCHRONIZE | STANDARD_RIGHTS_EXECUTE;
	if (access & GENERIC_ALL)
		access |= STANDARD_RIGHTS_ALL | CONSOLE_READ | CONSOLE_WRITE;
...
static int console_input_events_signaled(struct object *obj, struct w32thread *thread)
{
	...
	return obj->header.signal_state;
}
									...
									struct console_renderer_event* evt)
{
	...
	if (!collapsed) {
		if (evts->num_used == evts->num_alloc) {
			int old_size = evts->num_alloc;
			evts->num_alloc += 16;
			evts->events = realloc(evts->events, evts->num_alloc * sizeof(*evt), old_size * sizeof(*evt));
		}
...
static struct console_input_events *create_console_input_events(void)
{
	...
	status = create_object(KernelMode,
			console_input_events_object_type,
			NULL /* obj_attr*/,
			KernelMode,
			NULL,
			sizeof(struct console_input_events),
			0,
			0,
			(PVOID *)&evt);

	if (NT_SUCCESS(status) && evt) {
		INIT_DISP_HEADER(&evt->obj.header, CONSOLE_INPUT_EVENTS,
				sizeof(struct console_input_events) / sizeof(ULONG), 0);
		BODY_TO_HEADER(&(evt->obj))->ops = &console_input_events_ops;
	}
	else
		return NULL;
...
static struct object *create_console_input(struct w32thread* renderer)
{
	struct console_input *console_input;
	NTSTATUS status;

	status = create_object(KernelMode,
			console_input_object_type,
			NULL /* obj_attr*/,
			KernelMode,
			NULL,
	...
	if (NT_SUCCESS(status) && console_input) {
		INIT_DISP_HEADER(&console_input->obj.header, CONSOLE_INPUT,
				sizeof(struct console_input) / sizeof(ULONG), 0);
		BODY_TO_HEADER(&console_input->obj)->ops = &console_input_ops;
	}
	else
		return NULL;
...
static struct screen_buffer *create_console_output(struct console_input *console_input)
{
	...
	if (NT_SUCCESS(status) && screen_buffer) {
		INIT_DISP_HEADER(&screen_buffer->obj.header, SCREEN_BUFFER,
				sizeof(struct screen_buffer) / sizeof(ULONG), 0);
		BODY_TO_HEADER(&screen_buffer->obj)->ops = &screen_buffer_ops;
	}
	else
		return NULL;
	...
	for (i = 0; i < screen_buffer->width; i++)
		screen_buffer->data[i] = empty_char_info;
...
int free_console(struct w32process *process)
{
	struct console_input* console = process->console;

	if (!console || !console->renderer)
		return 0;
 ...
 */
void inherit_console(struct w32thread *parent_thread, struct w32process *process, obj_handle_t hconin)
{
	int done = 0;
	struct w32process* parent = parent_thread->process;
	...
	if (hconin) {
		...
		if ((console = (struct console_input*)get_wine_handle_obj(parent, hconin,
						0, &console_input_ops))) {
...
struct w32thread *console_get_renderer(struct console_input *console)
{
...
static struct console_input* console_input_get(obj_handle_t handle, unsigned access)
{
	...
	if (handle)
		console = (struct console_input *)get_wine_handle_obj(get_current_w32process(), handle,
				access, &console_input_ops);
	else if (get_current_w32process()->console) {
		console = (struct console_input *)grab_object(get_current_w32process()->console);
	}

	if (!console && !get_error())
...
static int propagate_console_signal_cb(struct w32process *process, void *user)
{
	...
	if (process->console == csi->console && process->running_threads &&
		...
		LIST_FOR_EACH_ENTRY(thread, &process->thread_list, struct w32thread, proc_entry) {
			if (send_thread_signal(thread2ethread(thread), csi->signal))
				break;
		}
	}
	return FALSE;
...
static int get_console_mode(obj_handle_t handle)
{
	...
	if ((obj = get_wine_handle_obj(get_current_w32process(), handle, CONSOLE_READ, NULL))) {
		if (BODY_TO_HEADER(obj)->ops == &console_input_ops)
			ret = ((struct console_input *)obj)->mode;
		else if (BODY_TO_HEADER(obj)->ops == &screen_buffer_ops)
...
static int set_console_mode(obj_handle_t handle, int mode)
{
	...
	if (!(obj = get_wine_handle_obj(get_current_w32process(), handle, CONSOLE_WRITE, NULL)))
		return 0;
	if (BODY_TO_HEADER(obj)->ops == &console_input_ops) {
					...
					const INPUT_RECORD *records)
{
	...
	if (!(new_rec = realloc(console->records,
					(console->recnum + count) * sizeof(INPUT_RECORD),console->recnum * sizeof(INPUT_RECORD)))) {
	...
	if (!console->recnum && count)
		set_event(console->event, EVENT_INCREMENT, FALSE);
...
static int read_console_input(obj_handle_t handle, int count, int flush)
{
	...
	if (!(console = (struct console_input *)get_wine_handle_obj(get_current_w32process(), handle,
	...
	if (flush) {
		...
		old_size = console->recnum;
		if ((console->recnum -= count) > 0) {
			INPUT_RECORD *new_rec = realloc(console->records,
					console->recnum * sizeof(INPUT_RECORD), old_size * sizeof(INPUT_RECORD));
			if (new_rec)
				console->records = new_rec;
		}
		else {
				...
				const WCHAR *title, data_size_t len)
{
	...
	if (!(console = console_input_get(req->handle, CONSOLE_WRITE)))
		goto error;
	...
	if (req->mask & SET_CONSOLE_INPUT_INFO_ACTIVE_SB) {
		...
		screen_buffer = (struct screen_buffer *)get_wine_handle_obj(get_current_w32process(), req->active_sb,
						...
						int new_width, int new_height)
{
	...
	if (new_height > old_height) {
		...
		for (i = 0; i < new_width; i++)
			new_data[old_height * new_width + i] = empty_char_info;
		/* and blast it to the other rows */
						...
						const struct set_console_output_info_request *req)
{
	...
	if (req->mask & SET_CONSOLE_OUTPUT_INFO_SIZE) {
		...
		if (!change_screen_buffer_size(screen_buffer, req->width, req->height))
			return 0;
	...
	if (req->mask & SET_CONSOLE_OUTPUT_INFO_MAX_SIZE) {
		...
		if (get_current_w32process()->console != screen_buffer->input) {
			set_error(STATUS_INVALID_PARAMETER);
			return 0;
		}
							...
							enum char_info_mode mode, int wrap)
{
	...
	switch (mode) {
		case CHAR_INFO_MODE_TEXT:
			{
				WCHAR *data;
				int count = min((int)(end - src), (int)(get_reply_max_size() / sizeof(*data)));
						...
						int w, int h)
{
	...
	if (!(screen_buffer = (struct screen_buffer *)get_wine_handle_obj(get_current_w32process(), handle,
...
extern struct eprocess *process2eprocess(struct w32process *);
...
DECL_HANDLER(alloc_console)
{
	...
	struct w32process *renderer = get_current_w32process();
	struct console_input *console;
	...
	if ((console = (struct console_input*)create_console_input(current_thread))) {
		if ((in = alloc_handle(renderer, console, req->access, req->attributes))) {
			...
			close_handle(process2eprocess(renderer), in);
		}
...
{
	ktrace("\n");
	free_console(get_current_w32process());
}
...
DECL_HANDLER(get_console_renderer_events)
{
	...
	ktrace("\n");
	evt = (struct console_input_events *)get_wine_handle_obj(get_current_w32process(), req->handle,
...
DECL_HANDLER(open_console)
{
	...
	if (req->from == (obj_handle_t)0) {
		if (get_current_w32process()->console && get_current_w32process()->console->renderer)
			obj = grab_object((struct object*)get_current_w32process()->console);
	}
	else if (req->from == (obj_handle_t)1) {
		if (get_current_w32process()->console && get_current_w32process()->console->renderer &&
				get_current_w32process()->console->active)
			obj = grab_object((struct object*)get_current_w32process()->console->active);
	}
	else if ((obj = get_wine_handle_obj(get_current_w32process(), req->from,
...
DECL_HANDLER(get_console_input_info)
{
	...
	ktrace("\n");
	if (!(console = console_input_get(req->handle, CONSOLE_READ)))
		return;
	if (console->title) {
...
DECL_HANDLER(write_console_input)
{
	...
	ktrace("\n");
	reply->written = 0;
	if (!(console = (struct console_input *)get_wine_handle_obj(get_current_w32process(), req->handle,
...
DECL_HANDLER(create_console_output)
{
	...
	ktrace("\n");
	if (!(console = console_input_get(req->handle_in, CONSOLE_WRITE)))
		return;
	...
	if (screen_buffer) {
		...
		reply->handle_out = alloc_handle(get_current_w32process(), screen_buffer, req->access, req->attributes);
		release_object(screen_buffer);
	}
	release_object(console);
}
...
DECL_HANDLER(set_console_output_info)
{
	...
	ktrace("\n");
	if ((screen_buffer = (struct screen_buffer*)get_wine_handle_obj(get_current_w32process(), req->handle,
					CONSOLE_WRITE, &screen_buffer_ops))) {
...
DECL_HANDLER(send_console_signal)
{
	...
	ktrace("\n");
	group = req->group_id ? req->group_id : get_current_w32process()->group_id;

	if (!group)
		set_error(STATUS_INVALID_PARAMETER);
	else
		propagate_console_signal(get_current_w32process()->console, req->signal, group);
}

/* get console which renderer is 'current' */
static int cgwe_enum(struct w32process* process, void* user)
{
...
DECL_HANDLER(get_console_wait_event)
{
	...
	ktrace("\n");
	if (get_current_w32process()->console && get_current_w32process()->console->renderer)
		console = (struct console_input*)grab_object((struct object*)get_current_w32process()->console);
	else enum_processes(cgwe_enum, &console);

	if (console) {
		reply->handle = alloc_handle(get_current_w32process(), console->event, EVENT_ALL_ACCESS, 0);
		release_object(console);
	}
