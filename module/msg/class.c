...
struct window_class
{
	...
	struct w32process *process;      /* process owning the class */
...
static struct window_class *create_class(struct w32process *process, int extra_bytes, int local)
{
	struct window_class *class;

	if (!(class = mem_alloc(sizeof(*class) + extra_bytes - 1)))
		return NULL;

	class->process = (struct w32process *)grab_object(process);
	class->count = 0;
	class->local = local;
	...
	if (local)
		list_add_head(&process->classes, &class->entry);
	else
		list_add_before(&process->classes, &class->entry);
	return class;
}
...
void destroy_process_classes(struct w32process *process)
{
...
static struct window_class *find_class(struct w32process *process, atom_t atom, void *instance)
{
	struct list_head *ptr;
...
struct window_class *grab_class(struct w32process *process, atom_t atom,
...
DECL_HANDLER(create_class)
{
	...
	class = find_class(get_current_w32process(), atom, req->instance); 
	...
	if (!(class = create_class(get_current_w32process(), req->extra, req->local))) {
		release_global_atom(NULL, atom);
		return;
	}
	class->atom       = atom;
...
DECL_HANDLER(destroy_class)
{
	...
	if (!(class = find_class(get_current_w32process(), atom, req->instance)))
...
DECL_HANDLER(set_class_info)
{
	...
	ktrace("\n");
	class = get_window_class(req->window);
	if (!class)
		return;

	if (req->flags && class->process != get_current_w32process()) {
	...
	if (req->flags & SET_CLASS_ATOM) {
		if (!grab_global_atom(NULL, req->atom))
			return;
		release_global_atom(NULL, class->atom);
		class->atom = req->atom;
	}
	if (req->flags & SET_CLASS_STYLE)
		class->style = req->style;
	if (req->flags & SET_CLASS_WINEXTRA)
		class->win_extra = req->win_extra;
	if (req->flags & SET_CLASS_INSTANCE)
		class->instance = req->instance;
	if (req->flags & SET_CLASS_EXTRA)
		memcpy(class->extra_bytes + req->extra_offset,
