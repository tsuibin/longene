...
extern struct desktop *get_desktop_obj(struct w32process *process, obj_handle_t handle, unsigned int access);
...
struct window
{
	...
	struct w32thread   *thread;       /* thread owning the window */
...
static void link_window(struct window *win, struct window *previous)
{
	if (previous == WINPTR_NOTOPMOST) {
		if (!(win->ex_style & WS_EX_TOPMOST) && win->is_linked)
	...
	if (free == -1) {
		...
		if (win->prop_inuse >= win->prop_alloc) {
			...
			if (!(new_props = realloc(win->properties,
							sizeof(*new_props) * (win->prop_alloc + 16) , sizeof(*new_props) * win->prop_alloc))) {
				set_error(STATUS_NO_MEMORY);
...
{
	struct w32thread *thread = win->thread;

	if (!thread)
		return;
	if (thread->queue) {
		if (win->update_region)
			inc_queue_paint_count(thread, -1);
...
void destroy_window(struct window *win)
{
	...
	if (win == shell_listview)
		shell_listview = NULL;
	if (win == progman_window)
		progman_window = NULL;
	if (win == taskman_window)
		taskman_window = NULL;
...
struct w32process *get_top_window_owner(struct desktop *desktop)
{
...
{
	struct window *win = desktop->top_window;
	ktrace("win %p, thread %p\n", win, win->thread);
	if (win && win->thread)
		post_message(win->handle, WM_CLOSE, 0, 0);
}
					...
					atom_t atom, void *instance)
{
	...
	if (!(desktop = get_thread_desktop(current_thread, DESKTOP_CREATEWINDOW)))
		return NULL;

	if (!(class = grab_class(get_current_w32process(), atom, instance, &extra_bytes))) {
		release_object(desktop);
		return NULL;
	}

	if (!(win = mem_alloc(sizeof(*win) + extra_bytes - 1)))
		goto failed;
	...
	if (parent && parent->thread && parent->thread != current_thread && !is_desktop_window(parent))  {
		if (!attach_thread_input(current_thread, parent->thread))
			goto failed;
	}
	else { /* otherwise just make sure that the thread has a message queue */
		if (!get_current_queue() && !init_thread_queue(current_thread))
			goto failed; 
	}

	/* put it on parent unlinked list */
	if (parent)
		list_add_head(&parent->unlinked, &win->entry);
	else {
		INIT_LIST_HEAD(&win->entry);
		desktop->top_window = win;
		set_process_default_desktop(get_current_w32process(), desktop, current_thread->desktop);
	}
...
void destroy_thread_windows(struct w32thread *thread)
{
...
static struct window *get_desktop_window(struct w32thread *thread, int create)
{
...
int make_window_active(user_handle_t window)
{
	...
	win->last_active = win->handle;
	if ((owner = get_user_object(win->owner, USER_WINDOW)))
...
{
	if (win->thread)
		inc_queue_paint_count(win->thread, incr);
}
                                           ...
                                           struct user_handle_array *array)
{
	...
	LIST_FOR_EACH_ENTRY(ptr, &parent->children, struct window, entry) {
		...
		if (!add_handle_to_array(array, ptr->handle))
			return 0;
	}
	return 1;
}
...
static int all_windows_from_point(struct window *top, int x, int y, struct user_handle_array *array)
{
	...
	if (!(top->style & (WS_MINIMIZE|WS_DISABLED)) &&
		...
		if (!get_window_children_from_point(top, x, y, array))
			return 0;
	}
	/* now add window to the array */
	if (!add_handle_to_array(array, top->handle))
		return 0;
	return 1;
}

/* return the thread owning a window */
struct w32thread *get_window_thread(user_handle_t handle)
{
	struct window *win = get_user_object(handle, USER_WINDOW);
	if (!win || !win->thread)
		return NULL;
	return (struct w32thread *)grab_object(win->thread);
}
...
static struct window *find_child_to_repaint(struct window *parent, struct w32thread *thread)
{
	struct window *ptr, *ret = NULL;

	LIST_FOR_EACH_ENTRY(ptr, &parent->children, struct window, entry) {
		if (!(ptr->style & WS_VISIBLE))
			continue;
...
user_handle_t find_window_to_repaint(user_handle_t parent, struct w32thread *thread)
{
	...
	if (top_window->thread == thread && win_needs_repaint(top_window))
		win = top_window;
	else
...
static struct region *intersect_window_region(struct region *region, struct window *win)
{
	...
	if (!intersect_region(region, region, win->win_region))
		return NULL;
				...
				struct region *region, int offset_x, int offset_y)
{
	...
	LIST_FOR_EACH_ENTRY(ptr, &parent->children, struct window, entry) {
		...
		if (!(region = subtract_region(region, region, tmp)))
			break;
...
{
	while (win->parent && !is_desktop_window(win->parent))
		win = win->parent;
	return win;
}
...
static struct region *get_visible_region(struct window *win, unsigned int flags)
{
	...
	if ((flags & DCX_PARENTCLIP) && win->parent && !is_desktop_window(win->parent)) {
		...
		if (win->win_region && !intersect_window_region(region, win))
			goto error;
	}
	else {
	...
	if (flags & DCX_CLIPCHILDREN) {
		...
		if (!clip_children(win, NULL, region, offset_x, offset_y))
			goto error;
	}
...
static int get_window_visible_rect(struct window *win, rectangle_t *rect, int frame)
{
	...
	*rect = frame ? win->window_rect : win->client_rect;
	...
	while (win->parent) {
		win = win->parent;
		if (!(win->style & WS_VISIBLE) || win->style & WS_MINIMIZE)
			return 0;
		if (!is_desktop_window(win)) {
			offset_x += win->client_rect.left;
			offset_y += win->client_rect.top;
		...
		if (!intersect_rect(rect, rect, &win->client_rect))
			return 0;
		if (!intersect_rect(rect, rect, &win->window_rect))
			return 0;
	}
...
static struct region *crop_region_to_win_rect(struct window *win, struct region *region, int frame)
{
	...
	if (!get_window_visible_rect(win, &rect, frame))
		return NULL;
	...
	if (region) {
		...
		if (!intersect_region(tmp, region, tmp))
			goto done;
		if (is_region_empty(tmp))
			goto done;
...
static void crop_children_update_region(struct window *win, rectangle_t *rect)
{
	...
	LIST_FOR_EACH_ENTRY(child, &win->children, struct window, entry) {
		...
		if (child->window_rect.left >= rect->left &&
				...
				child->window_rect.bottom <= rect->bottom)
			continue;
		...
		if (!child->update_region)
			continue;
		if (!(tmp = create_empty_region()))
			continue;
		set_region_rect(tmp, rect);
		offset_region(tmp, -child->window_rect.left, -child->window_rect.top);
		if (intersect_region(tmp, child->update_region, tmp))
			set_update_region(child, tmp);
		else
			free_region(tmp);
	}
}
...
static void validate_parents(struct window *child)
{
	...
	while (win->parent) {
		...
		if (win->update_region && !(win->style & WS_CLIPCHILDREN)) {
			if (!tmp && !(tmp = create_empty_region()))
				return;
...
static void redraw_window(struct window *win, struct region *region, int frame, unsigned int flags)
{
	...
	if (flags & RDW_NOCHILDREN)
		return;
	if (win->style & WS_MINIMIZE)
		return;
	if ((win->style & WS_CLIPCHILDREN) && !(flags & RDW_ALLCHILDREN))
		return;

	if (!(tmp = crop_region_to_win_rect(win, region, 0)))
		return;
	...
	LIST_FOR_EACH_ENTRY(child, &win->children, struct window, entry) {
		if (!(child->style & WS_VISIBLE))
			continue;
		if (!rect_in_region(tmp, &child->window_rect))
			continue;
...
static unsigned int get_update_flags(struct window *win, unsigned int flags)
{
	...
	if (flags & UPDATE_NONCLIENT) {
		if ((win->paint_flags & PAINT_NONCLIENT) && win->update_region)
			ret |= UPDATE_NONCLIENT;
	}
	if (flags & UPDATE_ERASE) {
		if ((win->paint_flags & PAINT_ERASE) && win->update_region)
			ret |= UPDATE_ERASE;
	}
	if (flags & UPDATE_PAINT) {
		if (win->update_region) {
			if (win->paint_flags & PAINT_DELAYED_ERASE)
				ret |= UPDATE_DELAYED_ERASE;
			ret |= UPDATE_PAINT;
		}
	}
				...
				unsigned int flags, struct window **child)
{
	 ...
	 */
	if (!(flags & UPDATE_ALLCHILDREN) && !(win->style & WS_CLIPCHILDREN))
		return 0;
	...
	LIST_FOR_EACH_ENTRY(ptr, &win->children, struct window, entry) {
		...
		if ((ret = get_child_update_flags(ptr, NULL, flags, child)))
			break;
	}
	return ret;
}
				...
				unsigned int flags, struct window **child)
{
	...
	for (ptr = from_child; ptr; ptr = ptr->parent) {
		if (!(ptr->style & WS_VISIBLE) || (ptr->style & WS_MINIMIZE))
			from_sibling = ptr;
		if (ptr == win)
			break;
	}
	...
	if (flags & UPDATE_NOCHILDREN)
		return 0;
	if (!from_sibling) {
		if ((ret = get_child_update_flags(from_child, NULL, flags, child)))
			return ret;
				...
				const rectangle_t *client_rect, const rectangle_t *valid_rects)
{
	...
	if (visible && !(old_vis_rgn = get_visible_region(win, DCX_WINDOW)))
		return;
	...
	if (get_window_visible_rect(win, &rect, 0)) {
		...
		crop_children_update_region(win, NULL);

	if (swp_flags & SWP_NOREDRAW)
		goto done;  /* do not repaint anything */
	...
	if (frame_changed || client_changed) {
		...
		if (valid_rects) {
			...
			if (tmp) {
				...
				if (subtract_region(tmp, win_rgn, tmp))
					win_rgn = tmp;
				else
					free_region(tmp);
			}
				...
				unsigned int swp_flags)
{
	...
	if (!memcmp(visible_rect, &old_visible_rect, sizeof(old_visible_rect)))
		return;
	...
	if (!(old_vis_rgn = get_visible_region(win, DCX_WINDOW)))
		return;
	win->visible_rect = *visible_rect;
...
static void set_window_region(struct window *win, struct region *region, int redraw)
{
	...
	if (redraw && !is_visible(win))
		redraw = 0;

	if (redraw)
		old_vis_rgn = get_visible_region(win, DCX_WINDOW);
...
DECL_HANDLER(create_window)
{
	...
	if (!req->parent)
		parent = get_desktop_window(current_thread, 0); 
	else if (!(parent = get_window(req->parent)))
		return;

	if (req->owner) {
		if (!(owner = get_window(req->owner)))
			return;
		if (is_desktop_window(owner))
			owner = NULL;
			...
			while (!is_desktop_window(owner->parent))
				owner = owner->parent;
	}
	...
	if (!(win = create_window(parent, owner, atom, req->instance)))
		return;
...
DECL_HANDLER(set_parent)
{
	...
	if (!(win = get_window(req->handle)))
		return;
	if (req->parent && !(parent = get_window(req->parent)))
		return;
...
DECL_HANDLER(destroy_window)
{
	...
	ktrace("\n");
	win = get_window(req->handle);
	if (win) {
		if (!is_desktop_window(win))
			destroy_window(win);
		else if (win->thread == current_thread)
			detach_window_thread(win);
		else 
			set_error(STATUS_ACCESS_DENIED);
	}
...
DECL_HANDLER(get_desktop_window)
{
	...
	ktrace("\n");
	win = get_desktop_window(current_thread, req->force);
	if (win)
		reply->handle = win->handle;
}
...
DECL_HANDLER(set_window_owner)
{
	...
	if (!win)
		return;
	if (req->owner && !(owner = get_window(req->owner)))
		return;
	if (is_desktop_window(win)) {
		set_error(STATUS_ACCESS_DENIED);
		return;
...
DECL_HANDLER(get_window_info)
{
	...
	if (win) {
		...
		if (get_user_object(win->last_active, USER_WINDOW))
			reply->last_active = win->last_active;
		if (win->thread) {
...
DECL_HANDLER(set_window_info)
{
	...
	if (req->flags & SET_WIN_EXSTYLE) {
		...
		if (!win->is_linked)
			...
			win->ex_style = (req->ex_style & ~WS_EX_TOPMOST) | (win->ex_style & WS_EX_TOPMOST);
	}
	if (req->flags & SET_WIN_ID)
		win->id = req->id;
	if (req->flags & SET_WIN_INSTANCE)
		win->instance = req->instance;
	if (req->flags & SET_WIN_UNICODE)
		win->is_unicode = req->is_unicode;
	if (req->flags & SET_WIN_USERDATA)
		win->user_data = req->user_data;
	if (req->flags & SET_WIN_EXTRA)
		memcpy(win->extra_bytes + req->extra_offset,
...
DECL_HANDLER(get_window_children)
{
	...
	ktrace("\n");
	if (req->desktop) {
		struct desktop *desktop = get_desktop_obj(get_current_w32process(), req->desktop, DESKTOP_ENUMERATE);
		if (!desktop)
			return;
	...
	LIST_FOR_EACH_ENTRY(ptr, &parent->children, struct window, entry) {
		if (atom && get_class_atom(ptr->class) != atom)
			continue;
		if (req->tid && get_thread_id(ptr->thread) != req->tid)
			continue;
		total++;
	}
	reply->count = total;
...
DECL_HANDLER(get_window_children_from_point)
{
	...
	array.count = 0;
	array.total = 0;
	if (!all_windows_from_point(parent, req->x, req->y, &array))
		return;

	reply->count = array.count;
	len = min(get_reply_max_size(), array.count * sizeof(user_handle_t));
	if (len)
		set_reply_data_ptr(array.handles, len);
	else
		free(array.handles);
}
...
DECL_HANDLER(get_window_tree)
{
	...
	if (win->parent) {
		...
		reply->owner  = win->owner;
		if (win->is_linked) {
			if ((ptr = get_next_window(win)))
				reply->next_sibling = ptr->handle;
			if ((ptr = get_prev_window(win)))
				reply->prev_sibling = ptr->handle;
		}
		if ((ptr = get_first_child(parent)))
			reply->first_sibling = ptr->handle;
		if ((ptr = get_last_child(parent)))
			reply->last_sibling = ptr->handle;
	}
	if ((ptr = get_first_child(win)))
		reply->first_child = ptr->handle;
	if ((ptr = get_last_child(win)))
		reply->last_child = ptr->handle;
}
...
DECL_HANDLER(set_window_pos)
{
	...
	if (!win)
		return;
	if (!win->parent)
		flags |= SWP_NOZORDER;  /* no Z order for the desktop */
	...
	if (!(flags & SWP_NOZORDER)) {
		switch ((int)(unsigned long)req->previous) {
			...
			case -2:  /* HWND_NOTOPMOST */
				...
				break;
			default:
				if (!(previous = get_window(req->previous)))
					return;
		...
		if (previous == win)
			flags |= SWP_NOZORDER;  /* nothing to do */
	}
...
DECL_HANDLER(set_window_visible_rect)
{
	...
	win = get_window(req->handle);
	if (win)
		set_window_visible_rect(win, &req->visible, req->flags);
}
...
DECL_HANDLER(get_window_text)
{
	...
	if (win && win->text) {
		...
		if (len > get_reply_max_size())
			len = get_reply_max_size();
...
DECL_HANDLER(set_window_text)
{
	...
	if (win) {
		...
		if (len) {
			if (!(text = mem_alloc((len+1) * sizeof(WCHAR))))
				return;
...
DECL_HANDLER(get_update_region)
{
	...
	if (req->from_child) {
		...
		if (!(from_child = get_window(req->from_child)))
			return;
	...
	if (flags & UPDATE_DELAYED_ERASE) { /* this means that the previous call didn't erase */
		if (from_child)
			from_child->paint_flags |= PAINT_DELAYED_ERASE;
		else
			win->paint_flags |= PAINT_DELAYED_ERASE;
	}
	...
	if (reply->flags & (UPDATE_PAINT|UPDATE_INTERNALPAINT)) { /* validate everything */
		...
		if (reply->flags & UPDATE_NONCLIENT)
			validate_non_client(win);
...
DECL_HANDLER(update_window_zorder)
{
	...
	ktrace("\n");
	win = get_window(req->window);
	if (!win || !win->parent || !is_visible(win))
		return;  /* nothing to do */
	...
	LIST_FOR_EACH_ENTRY(ptr, &win->parent->children, struct window, entry) {
		...
		if (ptr->ex_style & WS_EX_TRANSPARENT)
			continue;
		if (!intersect_rect(&tmp, &ptr->visible_rect, &req->rect))
			continue;
...
DECL_HANDLER(set_window_property)
{
	...
	if (get_req_data_size()) {
		...
		set_property(win, req->atom, req->handle, PROP_TYPE_ATOM);
}
...
DECL_HANDLER(remove_window_property)
{
	...
	if (win) {
		atom_t atom = req->atom;
		if (get_req_data_size())
			atom = find_global_atom(NULL, get_req_data(),
					get_req_data_size() / sizeof(WCHAR));
		if (atom)
			reply->handle = remove_property(win, atom);
	}
}
	...
	if (win) {
		...
		if (get_req_data_size())
			atom = find_global_atom(NULL, get_req_data(),
					get_req_data_size() / sizeof(WCHAR));
		if (atom)
			reply->handle = get_property(win, atom);
	}
}
...
DECL_HANDLER(get_window_properties)
{
	...
	if (count > max)
		count = max;
	if (!count || !(data = set_reply_data_size(count * sizeof(*data))))
		return;
...
DECL_HANDLER(set_global_windows)
{
	...
	if (req->flags & SET_GLOBAL_SHELL_WINDOWS) {
		if (!get_new_global_window(&new_shell_window, req->shell_window))
			return;
		if (!get_new_global_window(&new_shell_listview, req->shell_listview))
			return;
	}
	if (req->flags & SET_GLOBAL_PROGMAN_WINDOW) {
		if (!get_new_global_window(&new_progman_window, req->progman_window))
			return;
	}
	if (req->flags & SET_GLOBAL_TASKMAN_WINDOW) {
		if (!get_new_global_window(&new_taskman_window, req->taskman_window))
			return;
	}
