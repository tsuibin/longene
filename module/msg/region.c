...
static inline rectangle_t *add_rect(struct region *reg)
{
	if (reg->num_rects >= reg->size - 1) {
		rectangle_t *new_rect = realloc(reg->rects, 2 * sizeof(rectangle_t) * reg->size, sizeof(rectangle_t) * reg->size);
		if (!new_rect) {
...
static inline int validate_rectangles(const rectangle_t *rects, unsigned int nb_rects)
{
	...
	for (ptr = rects, end = rects + nb_rects; ptr < end; ptr++) {
		if (ptr->left >= ptr->right || ptr->top >= ptr->bottom)
			return 0;  /* empty rectangle */
		if (ptr == end - 1)
			break;
		if (ptr[0].top == ptr[1].top) { /* same band */
			if (ptr[0].bottom != ptr[1].bottom)
				return 0;  /* not same y extent */
			if (ptr[0].right >= ptr[1].left)
				return 0;  /* not properly x ordered */
		}
		else { /* new band */
			if (ptr[0].bottom > ptr[1].top)
				...
				non_overlap_func_t non_overlap2_func)
{
	...
	if (!(new_rects = mem_alloc(new_size * sizeof(*newReg->rects))))
		return 0;
	...
	do {
		...
		if (r1->top < r2->top) {
			...
			if ((top != bot) && non_overlap1_func) {
				if (!non_overlap1_func(newReg, r1, r1BandEnd, top, bot))
					goto done;
			}
			...
			if ((top != bot) && non_overlap2_func) {
				if (!non_overlap2_func(newReg, r2, r2BandEnd, top, bot))
					goto done;
			}
		...
		curBand = newReg->num_rects;
		if (ybot > ytop) {
			if (!overlap_func(newReg, r1, r1BandEnd, r2, r2BandEnd, ytop, ybot))
				goto done;
		}
	...
	if ((newReg->num_rects < (newReg->size / 2)) && (newReg->size > 2)) {
		...
		if ((new_rects = realloc(newReg->rects, sizeof(*newReg->rects) * new_size,
						sizeof(*newReg->rects) * newReg->size))) {
			newReg->rects = new_rects;
			newReg->size = new_size;
		}
	}
...
static void set_region_extents(struct region *region)
{
	...
	while (pRect <= pRectEnd) {
		if (pRect->left < region->extents.left)
			region->extents.left = pRect->left;
		if (pRect->right > region->extents.right)
			region->extents.right = pRect->right;
		pRect++;
	}
}
...
struct region *create_empty_region(void)
{
	...
	if (!(region = mem_alloc(sizeof(*region))))
		return NULL;
...
struct region *create_region_from_req_data(const void *data, data_size_t size)
{
	...
	if (nb_rects == 1 && !memcmp(rects, &empty_rect, sizeof(empty_rect)))
		nb_rects = 0;
...
struct region *copy_region(struct region *dst, const struct region *src)
{
	...
	if (dst->size < src->num_rects) {
		rectangle_t *rect = realloc(dst->rects, src->num_rects * sizeof(*rect) , dst->size * sizeof(*rect));
		if (!rect) {
			set_error(STATUS_NO_MEMORY);
			return NULL;
		}
					...
					const struct region *src2)
{
	...
	if (!region_op(dst, src1, src2, intersect_overlapping, NULL, NULL))
		return NULL;
					...
					const struct region *src2)
{
	...
	if (!region_op(dst, src1, src2, union_overlapping,
				union_non_overlapping, union_non_overlapping))
		return NULL;
...
int rect_in_region(struct region *region, const rectangle_t *rect)
{
	...
	for (ptr = region->rects, end = region->rects + region->num_rects; ptr < end; ptr++) {
		if (ptr->top >= rect->bottom)
			return 0;
		if (ptr->bottom <= rect->top)
			continue;
		if (ptr->left >= rect->right)
			continue;
		if (ptr->right <= rect->left)
			continue;
		return 1;
	}
	return 0;
