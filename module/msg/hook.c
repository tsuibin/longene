...
struct hook
{
	...
	struct w32process  *process;  /* process the hook is set to */
	struct w32thread   *thread;   /* thread the hook is set to */
	struct w32thread   *owner;    /* owner of the out of context hook */
...
static WCHAR hook_table_type_name[] = {'H', 'o', 'o', 'k', '_', 'T', 'a', 'b', 'l', 'e', 0};

POBJECT_TYPE hook_table_object_type = NULL;
EXPORT_SYMBOL(hook_table_object_type);

static GENERIC_MAPPING hook_table_mapping =
{
...
VOID
init_hook_table_implement(VOID)
{
	OBJECT_TYPE_INITIALIZER ObjectTypeInitializer;
	UNICODE_STRING Name;

	memset(&ObjectTypeInitializer, 0, sizeof(ObjectTypeInitializer));
	init_unistr(&Name, hook_table_type_name);
	ObjectTypeInitializer.Length = sizeof(ObjectTypeInitializer);
	ObjectTypeInitializer.DefaultNonPagedPoolCharge = sizeof(struct hook_table);
	ObjectTypeInitializer.GenericMapping = hook_table_mapping;
	ObjectTypeInitializer.PoolType = NonPagedPool;
	ObjectTypeInitializer.ValidAccessMask = EVENT_ALL_ACCESS;
	ObjectTypeInitializer.UseDefaultObject = TRUE;
	create_type_object(&ObjectTypeInitializer, &Name, &hook_table_object_type);
}
...
static struct hook_table *alloc_hook_table(void)
{
	...
	status = create_object(KernelMode,
			hook_table_object_type,
			NULL /* obj_attr*/,
			KernelMode,
			NULL,
			sizeof(struct hook_table),
			0,
			0,
			(PVOID *)&table);

	if (NT_SUCCESS(status) && table) {
		INIT_DISP_HEADER(&table->obj.header, HOOK_TABLE, 
				sizeof(struct hook_table) / sizeof(ULONG), 0);
		BODY_TO_HEADER(&(table->obj))->ops = &hook_table_ops;

		for (i = 0; i < NB_HOOKS; i++) {
			INIT_LIST_HEAD(&table->hooks[i]);
			table->counts[i] = 0;
		}
	}
	return table;
...
static struct hook_table *get_global_hooks(struct w32thread *thread)
{
	struct hook_table *table;
...
static struct hook *add_hook(struct desktop *desktop, struct w32thread *thread, int index, int global)
{
	struct hook *hook;
	struct hook_table *table = global ? desktop->global_hooks : get_queue_hooks(thread);

	if (!table) {
		if (!(table = alloc_hook_table()))
			return NULL;
		if (global)
			desktop->global_hooks = table;
		else
			set_queue_hooks(thread, table);
	}
	if (!(hook = mem_alloc(sizeof(*hook))))
		return NULL;
	...
	hook->thread = thread ? (struct w32thread *)grab_object(thread) : NULL;
	hook->table  = table;
	hook->index  = index;
...
static struct hook *find_hook(struct w32thread *thread, int index, void *proc)
{
	struct list_head *p;
...
{
	if ((!hook->process || hook->process == get_current_w32process()) &&
			(!(hook->flags & WINEVENT_SKIPOWNPROCESS) || hook->process != get_current_w32process())) {
		if ((!hook->thread || hook->thread == current_thread) &&
				(!(hook->flags & WINEVENT_SKIPOWNTHREAD) || hook->thread != current_thread)) 
			return 1;
	}
	return 0;
}
	...
	while (hook) {
		if (hook->proc && run_hook_in_current_thread(hook)) {
			if (event >= hook->event_min && event <= hook->event_max) {
				if (hook->flags & WINEVENT_INCONTEXT)
					return hook;
...
static struct hook *get_next_hook(struct w32thread *thread, struct hook *hook, int event,
...
static void hook_table_destroy(struct object *obj)
{
	...
	for (i = 0; i < NB_HOOKS; i++) {
		while ((hook = get_first_hook(table, i)) != NULL)
			free_hook(hook);
	}
}
...
void remove_thread_hooks(struct w32thread *thread)
{
...
DECL_HANDLER(set_hook)
{
	...
	if (!(desktop = get_thread_desktop(current_thread, DESKTOP_HOOKCONTROL)))
		return;
	...
	if (req->id == WH_KEYBOARD_LL || req->id == WH_MOUSE_LL) {
		...
		if (!(module = memdup(get_req_data(), module_size)))
			goto done;
		global = 1;
	}
	else {
		...
		else if (!(module = memdup(get_req_data(), module_size)))
			goto done;
		global = 0;
	}

	if ((hook = add_hook(desktop, thread, req->id - WH_MINHOOK, global))) {
		hook->owner = (struct w32thread *)grab_object(current_thread);
		hook->process = process ? (struct w32process *)grab_object(process) : NULL;
...
DECL_HANDLER(start_hook_chain)
{
	...
	ktrace("\n");
	table = get_queue_hooks(current_thread);
	...
	if (!table || !(hook = get_first_valid_hook(table, req->id - WH_MINHOOK, req->event,
		...
		/* try global table */
		if (!(table = get_global_hooks(current_thread)) ||
	...
	if (hook->module)
		set_reply_data(hook->module, hook->module_size);
}
...
{
	struct hook_table *table;
	struct hook_table *global_hooks;
	int index = req->id - WH_MINHOOK;

	ktrace("\n");
	table = get_queue_hooks(current_thread);
	global_hooks = get_global_hooks(current_thread);
	if (req->id < WH_MINHOOK || req->id > WH_WINEVENT) {
		set_error(STATUS_INVALID_PARAMETER);
		return;
	}
	if (table)
		release_hook_chain(table, index);
	if (global_hooks)
		release_hook_chain(global_hooks, index);
}
...
DECL_HANDLER(get_hook_info)
{
	...
	reply->unicode = hook->unicode;
	if (hook->module)
		set_reply_data(hook->module, min(hook->module_size,get_reply_max_size()));
	if (run_hook_in_owner_thread(hook)) {
		reply->pid  = get_process_id(hook->owner->process);
		reply->tid  = get_thread_id(hook->owner);
	}
